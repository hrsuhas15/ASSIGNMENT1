...
def SevenLoopNaiveMethod(N, C, H, W, R, S, M, stride, inputFMap, weights):
    ...
    outputFMapPooled = average_pooling(outputFMapPadded)
...
print("--------Convolution by 7-loop naive implementation-------")
naiveConvTime, outputFMapPooled = SevenLoopNaiveMethod(N, C, H, W, R, S, M, stride, inputFMap, weights)
# Print time taken for naive convolution
print("Time taken for 7-loop naive convolution:", naiveConvTime)
# Check the shape of the output feature maps
print("Output feature maps shape after pooling: (N, M, H_out, W_out) =", len(outputFMapPooled), len(outputFMapPooled[0]), len(outputFMapPooled[0][0]), len(outputFMapPooled[0][0][0]))
print("----------------------------------------")